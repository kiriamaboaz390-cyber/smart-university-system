...
- 2026-09-10: handoff design prompt created at `docs/STAKEHOLDER_FEATURES_PROMPT.md` for the design agent that will produce the F1–F4 designs (manual attendance, calendar integration, student schedule visibility, late-status policy). It pins the hard rules (TDD with RED/GREEN evidence, verification battery, RBAC via `src/lib/rbac.ts`, audit-log on state changes, additive-only SQLite migrations, no new deps without approval), lists the exact files to read first, requires a decisions table with vetoable defaults, and requires the design session to end with a `docs:`-only commit. §11.3 step 0 points to it.
...
0. Run the design agent with `docs/STAKEHOLDER_FEATURES_PROMPT.md` (committed 2026-09-10): it
   produces the four feature designs (manual attendance, calendar integration, student
   schedule visibility, late-status policy) as Living Doc §9.x with a vetoable-defaults
   decisions table and an ordered TDD plan. The implementation of those designs follows the
   agent's output, then resume with step 1.
